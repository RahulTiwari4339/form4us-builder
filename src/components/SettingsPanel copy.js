...
export default function SettingsPanel({ step, updateSettings }) {
  ...
  const renderField = (field) => {
    ...
    switch (field.type) {
     ...
     case "list":
  return (
    <div className="mt-4 space-y-3" key={field.key}>
      ...
      {(value || []).map((opt, index) => {
        ...
        return (
          <div
            key={index}
            className="flex gap-3 items-start rounded-xl border bg-white p-3 shadow-sm hover:shadow-md transition"
          >
            ...
            <div className="flex-1">
              <input
                ...
                onChange={(e) => {
                  const copy = [...value];
                  copy[index] = { ...option, label: e.target.value };
                  change(field.key, copy);
                }}
              />
            </div>
